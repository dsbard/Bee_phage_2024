...
stacked_bar = ggplot(genus_melt, aes(clean_names, value, fill = variable)) + 
  ...
  scale_fill_manual(values = bacto_pal, name = "Genus") +
  theme_minimal() + 
...
## Combine
```{r}
legend = data.frame("Bee" = c("Bumble bee", "Honey bee")) %>% 
  ggplot(aes(Bee, 1, col = Bee)) +
  geom_point(size = 5) +
...
legends = cowplot::plot_grid(cowplot::get_legend(legend))
...
top = cowplot::plot_grid(left, right,
                         nrow = 1, rel_heights = c(5, 5), rel_widths = c(5, 5), label_size = 20)
...
pdf(file = "/Users/dinosbardellati/Desktop/PhD_stuff/projects/Bee_phage/5kb/reviewer_comments/coverM_redo/output_files/bacterial_community.pdf", width = 12.5, height = 10)
final
dev.off()
```
...
otu_phy = otu
vOTU = readRDS("/Users/dinosbardellati/Desktop/PhD_stuff/projects/Bee_phage/5kb/reviewer_comments/coverM_redo/output_files/5kb_phage_normalized.RDS")
