...
perc_temp_plot = ggplot(perc_temp_df, aes(x = Bee_type, y = perc_temp, fill = Bee_type)) +
  geom_boxplot() + 
...
pdf("/Users/dinosbardellati/Desktop/PhD_stuff/projects/Bee_phage/5kb/reviewer_comments/coverM_redo/output_files/percent_temperate.pdf", width = 12.75, height = 8.25)
perc_temp_plot
dev.off()
...
# (Intercept)               28.960      3.891  20.000   7.443 3.49e-07 ***
# BeeHoney bee              21.337      5.503  20.000   3.878 0.000936 ***
# TypeVirome               -18.063      7.782  20.000  -2.321 0.030958 *  
# BeeHoney bee:TypeVirome    3.393     11.005  20.000   0.308 0.760999    
...
#                         Estimate Std. Error      df t value Pr(>|t|)   
# (Intercept)               19.023      8.440  17.364   2.254  0.03740 * 
# BeeBumble bee            -18.561     11.936  17.364  -1.555  0.13799   
# TypeTotal                 34.318      9.032  16.000   3.800  0.00157 **
# BeeBumble bee:TypeTotal  -20.918     12.773  16.000  -1.638  0.12101    
```
# set up other
```{r}
# vOTU table
...
# Plot it!
occupancy_plot = ggplot(occupancy, aes(MeanRelAbundance, Occupancy)) + 
...
occupancy_2 = occupancy %>% 
  ...
  group_by(Both, Occupancy) %>% 
  dplyr::count() %>% 
  ungroup() %>% 
  mutate(pOTU = n/sum(n) * 100)

palette = palette(value = c("#d73027", "#4575b4"))
bin_perc = ggplot(occupancy_2, aes(Occupancy, pOTU, fill = Both, color = Both)) +
...
pdf("/Users/dinosbardellati/Desktop/PhD_stuff/projects/Bee_phage/5kb/reviewer_comments/coverM_redo/output_files/occ_abund.pdf", width = 16.5, height = 12.75)
cowplot::plot_grid(legends, top, bottom, nrow = 3, rel_heights = c(1, 10, 7), label_size = 20)
dev.off()
```